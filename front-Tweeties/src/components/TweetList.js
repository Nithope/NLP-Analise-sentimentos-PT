...
class TweetList extends React.Component {
  constructor(props) {
    super(props);
    this.state = { items: [], searchTerm: "",sentiments:[] };
  ...
  handleResume() {
    ...
    fetch("https://socketiosv.mybluemix.net/setSearchTerm",
      {
        method: "POST",
        headers: {
  ...
  handlePause(event) {
    fetch("https://socketiosv.mybluemix.net/pause",
      {
        method: "POST",
        headers: {
          'Content-Type': 'application/json'
...
componentDidMount() {
  const socket = socketIOClient('https://socketiosv.mybluemix.net/');
  ...
  socket.on('connect', () => {
    ...
    socket.on("sentiment", sentiment => {
      ...
      let newList = [sentiment].concat(this.state.sentiments.slice(0, 15));
      this.setState({ sentiments: newList });
    });
  });
  socket.on('disconnect', () => {
    ...
    socket.removeAllListeners("sentiment");
